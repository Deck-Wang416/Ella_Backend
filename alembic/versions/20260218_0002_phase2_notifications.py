...
def upgrade() -> None:
    ...
    if "uq_subscription_unique_endpoint" not in sub_indexes:
        op.create_index(
    ...
    log_cols = {col["name"] for col in inspector.get_columns("notification_logs")}
    if "delivered_count" not in log_cols:
        op.add_column("notification_logs", sa.Column("delivered_count", sa.Integer(), nullable=False, server_default="0"))
    if "failed_count" not in log_cols:
